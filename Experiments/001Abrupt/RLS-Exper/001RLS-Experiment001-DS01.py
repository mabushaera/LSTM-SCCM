...
from Datasets.SyntheticDS.Abrupt import DS01
...
def TB_Experiment04(X_train, y_train, X_test, y_test):
    ...
    rls_lambda_1 = .99  # close to 1 means equal weights between past and new data
    rls_lambda_2 = .18  # close to 0 means forget old data and favor new data.
    rls_delta = .01
    ...
    kpi = 'MSE'
    multiplier_mse = 2.5
    # rls_lambda_1 start with equal weights, then LSTM will dynamically adjust it
    adrls_acc, adrls_mse_list = RLS_LSTM_SCCM.ad_rls_generic(X_train, y_train, rls_lambda_1, rls_delta,
                                                             X_test, y_test,
                                                             kpi=kpi,
                                                             multiplier=multiplier_mse, DS='DS1')
    ...
    if (plotting_enabled):
        ...
        label1 = 'RLS'
        label2 = 'RLS$^*$'
        Plotter.plot_results(x_axis, y_axis1, y_axis2, kpi, label1, label2, drift_location=500, log_enabled=False,
                             legend_loc='upper left', drift_type='abrupt',
