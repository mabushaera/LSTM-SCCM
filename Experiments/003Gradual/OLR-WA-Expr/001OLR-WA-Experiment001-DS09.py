...
from Datasets.SyntheticDS.Gradual import DS09
...
def TB_Experiment01(X_train, y_train, X_test, y_test):
    ...
    if (plotting_enabled):
        mini_batch_size = Hyperparameter.olr_wa_increment_size(n_features, user_defined_val=10)
        ...
        label1 = 'OLR-WA'
        label2 = 'OLR-WA$^*$'
        Plotter.plot_results(x_axis, y_axis1, y_axis2, kpi, label1, label2, drift_location=100,
                             log_enabled=False, legend_loc='lower left',
                             drift_type='gradual',
                             gradual_drift_locations=[250, 350, 450, 650, 750, 1000],
